...
function ResponsiveTable() {
  ...
  const handleEdit = (tracking) => {
    navigate(`/trackingform`, { state: { existingTracking: tracking } }); // Redirect to the form with the tracking's ID
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await fetch(`${apiUrl}/tracking/${deleteId}`, {
        method: 'DELETE',
        headers: {
          "Content-Type": "application/json",
        }
  ...
  const filteredRows = rows.filter(
    (row) =>
      row.userId && typeof row.userId === "object"  ? row.userId.name.toLowerCase().includes(searchText.toLowerCase()) : "" ||
                ...
                {filteredRows
                  ...
                  .map((row) => (
                    <TableRow key={row._id}>
                      ...
                      <TableCell>
                        {typeof row.carId === "object" ? row.carId.code : row.carId.code || ""}
                      </TableCell>
                      <TableCell>
